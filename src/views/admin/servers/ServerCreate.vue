...
async function handleCreate() {
  error.value = ''
  const { response } = await adminClient.POST('/servers', {
    body: { id: id.value, plainPassword: plainPassword.value },
  })
  if (response.ok) {
    router.push('/admin/servers')
  } else if (response.status === 409) {
    error.value = '이미 존재하는 서버 ID입니다.'
  } else {
    error.value = '서버 생성에 실패했습니다.'
  }
}
</script>
...
<template>
  <div>
    <h1 class="text-2xl font-bold mb-6">서버 생성</h1>
